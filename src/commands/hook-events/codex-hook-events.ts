...
function mapCodexRolloutActivityLine(line: string): { mapped: CodexMappedHookEvent; fingerprint: string } | null {
	...
	if (lineType === "response_item") {
		...
		if (payloadType === "function_call") {
			...
			const requestsUserInput = isCodexUserInputToolName(name);
			const event: RuntimeHookEvent = requestsUserInput ? "to_review" : "activity";
			const activityText = requestsUserInput
				? "Waiting for input"
				: command
