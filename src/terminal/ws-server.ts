...
export interface CreateTerminalWebSocketBridgeRequest {
	...
	/** Maximum time one viewer may keep the shared PTY paused. */
	maxViewerPauseMs?: number;
	/** Maximum live output buffered while a viewer is applying its restore snapshot. */
...
const TERMINAL_VIEWER_MAX_PAUSE_MS = 2_000;
const TERMINAL_VIEWER_MAX_PENDING_OUTPUT_BYTES = 512 * 1024;
...
export function createTerminalWebSocketBridge({
	...
	const resolvedMaxViewerPauseMs = maxViewerPauseMs ?? TERMINAL_VIEWER_MAX_PAUSE_MS;
	const resolvedMaxPendingOutputBytes = maxPendingOutputBytes ?? TERMINAL_VIEWER_MAX_PENDING_OUTPUT_BYTES;
	...
	const createIoOutputState = (
		...
		let maxPauseTimer: ReturnType<typeof setTimeout> | null = null;
		...
		const clearMaxPauseTimer = () => {
			if (maxPauseTimer !== null) {
				clearTimeout(maxPauseTimer);
				maxPauseTimer = null;
			}
		};
		...
		return {
			...
			acknowledgeOutput: (bytes: number) => {
				...
				// Ack progress rearms the stall watchdog. The separate absolute pause
				// budget still prevents any viewer from blocking the PTY for too long.
		...
		streamState.detachOutputListener = terminalManager.attach(taskId, {
			onOutput: (chunk) => {
				for (const viewerState of streamState.viewers.values()) {
					...
					if (viewerState.pendingOutputBytes + chunk.byteLength > resolvedMaxPendingOutputBytes) {
						viewerState.pendingOutputChunks = [];
						viewerState.pendingOutputBytes = 0;
						viewerState.ioSocket?.terminate();
						viewerState.controlSocket?.terminate();
						continue;
					}
