...
describe("createTerminalWebSocketBridge", () => {
	...
	it("bounds how long one viewer can pause the shared PTY", async () => {
		const ioUrl = `${runtimeUrl}/api/terminal/io?taskId=${TASK_ID}&workspaceId=${WORKSPACE_ID}&clientId=slow`;
		const controlUrl = `${runtimeUrl}/api/terminal/control?taskId=${TASK_ID}&workspaceId=${WORKSPACE_ID}&clientId=slow`;
		...
		terminalManager.emitOutput(TASK_ID, "x".repeat(120_000));
	...
	it("disconnects a viewer whose pre-restore output buffer exceeds its budget", async () => {
		const ioUrl = `${runtimeUrl}/api/terminal/io?taskId=${TASK_ID}&workspaceId=${WORKSPACE_ID}&clientId=restoring`;
		const controlUrl = `${runtimeUrl}/api/terminal/control?taskId=${TASK_ID}&workspaceId=${WORKSPACE_ID}&clientId=restoring`;
		const ioSocket = await openQueuedWebSocket(ioUrl);
		const controlSocket = await openQueuedWebSocket(controlUrl);

		await waitForControlMessage(controlSocket, (message) => message.type === "restore");
		terminalManager.emitOutput(TASK_ID, "x".repeat(600 * 1_024));

		await waitForAssertion(() => {
			expect(ioSocket.socket.readyState).toBe(WebSocket.CLOSED);
			expect(controlSocket.socket.readyState).toBe(WebSocket.CLOSED);
		}, 2_000);
		expect(terminalManager.pauseOutput).not.toHaveBeenCalled();
	});
