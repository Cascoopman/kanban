...
describe("inferHookSourceFromPayload", () => {
	it("infers claude from unix transcript paths", () => {
		expect(
	...
	it("infers claude from windows transcript paths", () => {
		expect(
			inferHookSourceFromPayload({
	...
	it("infers codex from agent completion events", () => {
		expect(
			inferHookSourceFromPayload({
	...
	it("prefers Claude transcript paths over Codex event fallback", () => {
		expect(
			inferHookSourceFromPayload({
				transcript_path: "C:\\Users\\dev\\.claude\\projects\\task\\transcript.jsonl",
				type: "agent-turn-complete",
