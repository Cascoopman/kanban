...
- When the user must act within a few minutes, use the separate `notify-user-mac` MCP's `notify_user_mac` tool with an urgent title, concrete deadline, and concrete action. It sends both a Notification Center entry and a modal alert that remains visible under Focus modes. Also report the issue in the conversation, and do not use notifications for routine progress or completion updates.
...
- Application logging deliberately mirrors existing output instead of adding semantic instrumentation. Keep browser console output and runtime stdout/stderr in separate files under `$KANBAN_RUNTIME_HOME/logs` for local debugging, but do not expose logs through the agent interface or persist the remote-access passcode.
