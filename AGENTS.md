...
- Workspace discovery is non-destructive: a missing directory or failed Git probe means a workspace is registered-but-unavailable, not deleted. Keep its persisted board/session state visible as a fallback and return an actionable unavailable-workspace error; only an explicit project-removal action may delete that state.
