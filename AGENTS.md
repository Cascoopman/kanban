...
- Persisted state under `~/.kanban` is shared by production and preview runtimes unless the runtime home is explicitly isolated. Use `npm run dev:isolated -- --agent codex` (or `claude`) for manual testing: it sets a temporary `KANBAN_RUNTIME_HOME`, uses a disposable Git project, selects fresh random ports for a distinct browser origin, and shows an `Isolated preview` badge. Never launch an ordinary preview against shared production state. After a persisted-schema migration, never launch an older checkout against that shared state: refresh/rebuild from current `main` first. A stale runtime can otherwise rewrite newer board data using its older schema.
