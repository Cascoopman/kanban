...
- Tasks persist only a required title and task settings; they do not persist prompts or images. The new-task UI collects a prompt and optional images, derives an initial title, starts the task immediately, and forwards that input only to the first agent turn.
- Remote refs for task base resolution are fetched when a workspace first connects and again for each new task. Reuse an OpenSSH control connection within a project's main worktree and task worktrees, but isolate control sockets by Git common directory so credentials are never reused across projects.
- Persisted state under `~/.kanban` is shared by production and preview runtimes unless the runtime home is explicitly isolated. After a persisted-schema migration, never launch an older checkout against that shared state: refresh/rebuild from current `main` first. A stale runtime can otherwise rewrite newer board data using its older schema.
