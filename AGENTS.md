...
- When `KANBAN_TASK_ID` is present, begin with the `kanban_task_current` MCP tool to identify the card and confirm its current title and state. Do not use a `kanban task` CLI fallback.
- Treat the task title as agent-owned metadata. If it is generic, unclear, or no longer reflects the work, update it early with `kanban_task_update` and a concise outcome-oriented title; inside a task session the MCP infers the task and workspace automatically.
- When the user must act within a few minutes, immediately call `kanban_notify` with a concise title, deadline subtitle, and concrete action. It sends both a Notification Center entry and a modal alert that remains visible under Focus modes. Also report the issue in the conversation, and do not use notifications for routine progress or completion updates.
...
- Do not rewrite a task worktree's `AGENTS.md`, because that dirties tracked repositories and creates untracked files in repositories without one. Codex uses its native global `~/.codex/AGENTS.md` and project `AGENTS.md` discovery; Kanban does not inject a separate global instruction layer.
- Application logging deliberately mirrors existing output instead of adding semantic instrumentation. Keep browser console output and runtime stdout/stderr in separate files under `$KANBAN_RUNTIME_HOME/logs`, expose bounded snapshots through `kanban_logs`, and never persist the remote-access passcode.
- npm global binaries invoke package entrypoints through symlinks. MCP entrypoint guards must compare `import.meta.url` with the resolved real path of `process.argv[1]`, not the symlink path, or `kanban-mcp` will exit before opening its stdio transport even though `node dist/mcp.js` works.
