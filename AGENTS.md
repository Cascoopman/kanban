...
- When the user must act within a few minutes, use the separate `kanban-notify` MCP's `kanban_notify` tool with an urgent title, concrete deadline, and concrete action. It sends both a Notification Center entry and a modal alert that remains visible under Focus modes. Also report the issue in the conversation, and do not use notifications for routine progress or completion updates.
